...
console.log("\nThen, in order — the tag is not optional, version-agrees.test.ts");
console.log("checks for it, because a version with nothing to check out is a");
console.log("string rather than a release:");
console.log("  cd apps/ios && xcodegen        # Info.plists pick the number up");
console.log(`  git commit -am "chore: v${next}"`);
